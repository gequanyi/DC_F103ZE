 ...
 */
 ...
 //         读写、延时、打印函数的接入
 //         进行相关C99语法修改
 //         注释无用的但会编译错误的语句，宏定义，寄存器
 //         抽取montion_driver_test.c部分函数，在文末增加部分函数
 ////////////////////////////////////////////////////////////////
...
//    unsigned char accel_cfg2;            注释掉的设备MPU6050不支持
...
#if defined MPU6050              //MSP340使用C99语法，而Keil不是
const struct gyro_reg_s reg = {
0x75,  //who_am_i
0x19,  //rate_div
0x1A,  //lpf
 ...
 *  @return     0 if successful.
 */
int mpu_read_reg(unsigned char reg, unsigned char *data)                         //向芯片读寄存器的值除了MEMERY     FIFO
{
 ...
 *  @return     0 if successful.
 */
int mpu_init(void)                                    //MPU6050初始化    传入参数删除
{
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)   //获得新的原始陀螺仪数据
{
    unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_reg(short *data, unsigned long *timestamp)  //获得新的原始加速度数据
{
    unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(long *data, unsigned long *timestamp)  //获得新的温度数据
{
    unsigned char tmp[2];
    short raw;
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias(const long *accel_bias)  //配置偏差函数
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_lpf(unsigned short lpf)  //配置DLFP范围函数
{
    unsigned char data;
 ...
 *  @return     0 if successful.
 */
int mpu_set_sample_rate(unsigned short rate)   //设置采样频率范围函数
{
    unsigned char data;
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_sens(float *sens)      //陀螺仪灵敏度比例因子
{
    switch (st.chip_cfg.gyro_fsr) {
    case INV_FSR_250DPS:
        sens[0] = 131.f;
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_sens(unsigned short *sens)         //加速计灵敏度比例因子
{
    switch (st.chip_cfg.accel_fsr) {
    case INV_FSR_2G:
        sens[0] = 16384;
 ...
 *  @return     0 if successful.
 */
int mpu_set_sensors(unsigned char sensors)           //配置传感器的时钟和工作状态
{
    unsigned char data;
#ifdef AK89xx_SECONDARY
    unsigned char user_ctrl;
 ...
 *  @return     0 if successful.
 */
int mpu_get_int_status(short *status)       //获得中断状态
{
    unsigned char tmp[2];
    if (!st.chip_cfg.sensors)
        return -1;
...
int mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,                
        unsigned char *sensors, unsigned char *more)                   //获得FIFO数据
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_bypass(unsigned char bypass_on)    //设置路旁模式
{
    unsigned char tmp;
 ...
 *  @return     0 if successful.
 */
int mpu_set_int_level(unsigned char active_low)     //设置中断优先级
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_int_latched(unsigned char enable)    //设置中断锁存
{
    unsigned char tmp;
...
static int accel_self_test(long *bias_regular, long *bias_st)  //自检
{
    int jj, result = 0;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)      //获得所有偏差值
{
    unsigned char data[MAX_PACKET_LENGTH];
 ...
 */
int mpu_run_self_test(long *gyro, long *accel)           //行自检值
{
#ifdef MPU6050
...
int mpu_load_firmware(unsigned short length, const unsigned char *firmware,
    unsigned short start_addr, unsigned short sample_rate)                    //加载并验证DMP映像
{
    unsigned short ii;
 ...
 *  @return     0 if successful.
 */
int mpu_set_dmp_state(unsigned char enable)             //DMP状态控制
{
    unsigned char tmp;
 ...
 *  @return     0 if successful.
 */
int mpu_get_dmp_state(unsigned char *enabled)          //DMP状态读取
{
